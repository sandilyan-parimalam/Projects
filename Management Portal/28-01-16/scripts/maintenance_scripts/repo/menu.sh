...
if [ "$action" == "auto_lin_update" ]; then
LINUpdateAuto
exit
fi
...
function GetBuildInfo() {
        ...
        if [ -d "/usr/local/${instance}/webapps/lin" ] ; then
                ...
                pid=`ps -aef | grep /${instance}/ | grep -v grep | head -1 | awk '{print $2}'`
                if [ ! -z "$pid" ] ; then
                       echo " Seems the instance is online, Please stop it and continue"
                        log $action $service "--> Failed --> No Changes Done"
                       exit
                else
                        echo -e " Instace is OFFLINE, good to go... "
                fi

                service=$instance

        else
                echo -e "selected instance seems invalid, please select valid one.. "
                echo -e " if you sure the selected instance is correct, examin the script..."
                log $action $service "--> Failed --> No Changes Done"
                exit
        fi

if [ -z "$ftp_path" ] ; then
        echo -e "Empty FTP path..... "
        log $action $service "--> Failed --> No Changes Done"
        exit
fi

if [ -z "$file_name" ] ; then
        echo -e "Empty file_name..... \n"
        log $action $service "--> Failed --> No Changes Done"
        exit
        ...
        if [ ! `echo $file_name | grep -i "war"` ] ; then
                echo -e " \n provide a valid file name with .WAR extention \n"
                log $action $service "--> Failed --> No Changes Done"
                exit
        fi
...
End-Of-Session
        ...
        if [ -f "$temp_dir/$file_name" ] ; then
                ...
                size_of_war=`du "$temp_dir/$file_name" | awk '{print $1}'`
                if [ $size_of_war -le 0 ] ; then
                        echo -e " Seems the war file is empty, please check...."
                        log $action $service "--> Failed --> No Changes Done"
                        exit
                fi
                ...
                log $action $service "--> Failed --> No Changes Done"
                if [ "`cat $ftp_log | grep -i "Not connected"`" ] ; then
                        echo -e " Unable to connect FTP Server, please check ftp servername or ipaddress in conf file and check the FTP is UP "
                        exit
                elif [ "`cat $ftp_log | grep -i "Login failed"`" ] ; then
                        echo -e "Unable to login with the credentials stored in conf file, please check  the username/password or it may be changed/locked"
                        exit
                elif [ "`cat $ftp_log | grep -i "Failed to change directory"`" ] ; then
                        echo -e "Given FTP Path $ftp_path is not exist on FTP, please check and provide valid case sensitive path "
                        exit
                elif [ "`cat $ftp_log | grep -i "Failed to open file"`" ] ; then
                        echo -e "Given file name $file_name is not exist on FTP, please check and provide valid case sensitive patch name.... "
                        exit
                else
                        echo -e "Somehow unable to download $file_name, please check the drive space and other reasons.. Below Is the ftp log"
                        cat $ftp_log
                        exit
                fi
...
if [ $? -eq 0 ] ; then
        ...
        echo -e " Unable to backup the existing ${instance},Please check...terminatting"
        log $action $service "--> Failed --> No Changes Done"
        exit
...
if [ $? -eq 0 ] ; then
        ...
        if [ $? -eq 0 ] ; then
                ...
                if [ $? -eq 0 ] ; then
                        ...
                        echo -e " Done....."
                        echo "LIN UPDATE successful....."
                        echo -e "Please validate and start the instance...."
                        log $action $instance "--> Success"
                        exit

                else

                        echo -e "Unable to Change the ownership of /usr/local/${instance} to www:www , Please check and do it manualy..."
                        echo -e "once you changed the ownership, LIN UPDATE will be completed... "
                        log $action $service "--> Partially Failed --> Need to change the ownership of the instance dir to comeplete the update"
                        exit
                ...
                echo -e " Jar command Failed...."
                echo -e " Please make sure /usr/local/java/bin/jar is available , if not create a simbolic link and try again...."
                echo -e "if the file/command is already there, please refer the above jar command error and act accordingly...."
                echo -e " Terminating the script, Please revert the backup, if needed. Backup detail is above.....\n"
                log $action $service "--> Failed --> Changes Happen --> Needs to revert --> Backup is ${backup_dir}/${instance}_bkp_${dtf}.tgz"
                exit
        ...
        echo -e "Something went wrong while removing old files, Please refer the above rm command error and act accordingly...."
        log $action $service "--> Failed --> Changes Happen --> Needs to revert --> Backup is ${backup_dir}/${instance}_bkp_${dtf}.tgz"
        exit
fi
        ...
        echo -e "Unable to get in to the instance's lin directory ( 'cd /usr/local/${instance}/webapps/lin' --> failed ), Please Check..."
        echo "NO CHNAGES done so far to revert..."
        log $action $service "--> Failed --> No Changes Happen"
        exit
fi
		...
		if [ "$action" == "status" ] || [ "$action" == "stop" ] || [ "$action" == "start" ] || [ "$action" == "restart" ] || [ "$action" == "linupdate" ] ; then
			if [ "$action" == "linupdate" ] ; then
				...
				if  [ ! -z "$3" ] && [ ! -z "$4" ] ; then
					...
					echo "required info not detected for LIN Deployment/update, below are the requirements"
					echo "./scriptname.sh <linupdate> <tomcat instance name> <ftp path of the build> <war file name>"
				fi
