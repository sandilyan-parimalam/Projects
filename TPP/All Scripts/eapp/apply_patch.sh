	...
	if [ -f "$base_dir/tppenv.conf" ] ; then
		...
		if [ "$app_home" ] || [ "$rpatch_home" ] || [ "$SNAME" ] || [ "$UNAME" ] || [ "$PASSWORD" ] ; then
			...
			if [ -d "$app_home" ] ; then instance_base_dir=$app_home ; else echo -e "\033[31m Invalid app_home configured in tppenv.conf, Please change it..\033[0m"; exit;fi
                        if [ ! -d "$rpatch_home" ] ; then echo "Creating reverse patch home $rpatch_home" ;fi
...
select instance in $( ls /etc/init.d/jboss* | egrep -v 'old|don|bk|backup' | cut -d "-" -f2- )
do

if [ ! -d "$instance_base_dir/$instance/deployments" ] ; then
...
if [ $? -eq 0 ] ; then
        echo -e "\033[32;1m \n Files are placed successfully \033[0m"
        echo -e "\n Changing ownership, please wait"
        chown -R jboss:jboss $instance_home
        if [ $? -eq 0 ] ; then
                echo -e "\033[32;1m \n Done...\n"
                echo -e "\033[32;1m Patch applied successfully, Please start or restart the application if needed...."
                exit
        esle
                echo -e "\033[31m Facing errors while changing ownership, please CHANGE ownership manualy for the instance home directory, use the below command"
                echo "chown -R jboss:jboss $instance_home"
                echo -e "\033[32;1m Patch applied successfully, Please start or restart the application if needed...."
                exit
