...
echo "
<table>
<tr><td><b> Client  </td> <td><b>  Service Details </td> <td>Container</td><td>Space</td><td>Performance</td></tr>
...
> ${base_dir}/work/${datentime}_tmp.txt
if [ "${CSTATE}" == "running" ] ; then
for i in `ls /vz/root/${CID}/etc/init.d/ | egrep  '^apache|lin|^keepalive*' | egrep -v 'old|new|don|tmp|bk|not|bkp|backup|z'| sed 's#test_lin#tomcat_test#g' | sed 's#apache#apache/tomcat#g'| sort`
do
...
echo "<p align=center>Apache is  $apache_status<br>Tomcat is $tomcat_status" >> ${base_dir}/work/${datentime}_tmp.txt
...
echo "${Service_Name} is ${status}" >> ${base_dir}/work/${datentime}_tmp.txt
...
echo "<p align=center >Not Available </p>" > ${base_dir}/work/${datentime}_tmp.txt
fi
...
<td ><pre>`cat ${base_dir}/work/${datentime}_tmp.txt`</p></pre></td>
