...
function Go(Client){
	...
	var option = prompt("You are going to "+SelectedAction+" a service of " +Client+ " on aigtest \ncommand : /etc/init.d/"+ScriptName+" "+SelectedAction+"\nAre you sure???", "give yes or no" );

	if ( option == "yes" ) {

	var myWindow = window.open('LXCommandTracer.php?hst=aigtestnew&scriptname='+ScriptName+'&action='+SelectedAction+' ', "", "dialog=yes,location=no,scrollbars=yes,toolbar=no,menubar=no,width=600, top=100, left=500, height=500");
	}
 ...
 $("#maindiv").load('GenerateTable.php?srv=aigtestnew');
    })
...
<body background="/eh-portal/images/background.jpg">
        ...
        <p align=right><font color=green><b><strong>Welcome <?php echo $_SESSION['login_user']; ?> <font color=red>| </font><a href="LogOut.php?username=<?php echo $_SESSION['login_user']; ?>" style="text-decoration:none;color:green;"> Logout  </a></strong></b></font> </p>
			...
			<h2 align=center> <font color=green></i>AIGTEST Management Portal</i></font></h2>
