...
<script>
 ...
 $("#maindiv").load('GenerateTable.php?srv=lin4vz');
    })
</script>
<script>
...
function PopUp(cname, cid, svc, action) {
	...
	if ( option == "yes" && cname && cid && svc && action) {
		var myWindow = window.open('execution_manageSVC_popup.php?hst=lin4vz&&CNAME='+ cname +'&&CID='+ cid +'&&SVC='+ svc +'&&ACTION='+ action+' ', "", "dialog=yes,toolbar=no,menubar=no,width=500, top=100, left=500, height=500");
	}
}	
...
if ( state != "running" ) {
                ...
                if ( opt == "yes" && contid && clientname && state) {
                        myWindow = window.open('execution_manageContainer_popup.php?hst=lin4vz&CID='+ contid +'', "", "dialog=yes,toolbar=no,menubar=no,width=500, top=100, left=500, height=500");
                }
}
...
<body background="/eh-portal/images/background.jpg">
        ...
        <p align=right><font color=green><b><strong>Welcome <?php echo $_SESSION['login_user']; ?> <font color=red>| </font><a href="LogOut.php?username=<?php echo $_SESSION['login_user']; ?>" style="text-decoration:none;color:green;"> Logout  </a></strong></b></font> </p>
			...
			<h2 align=center> <font color=green></i>ASPLIN4VZ Management Portal</i></font></h2>

<ul>
  <li><a class="active" href="lin4vz_portal.php">Controll Panel</a></li>
  <li><a href="LinDeployment.php?hst=lin4vz" target="_blank">Lin Deployment</a></li>
  <li><a href="lin4vz_portal.php">Reload Data</a></li>
